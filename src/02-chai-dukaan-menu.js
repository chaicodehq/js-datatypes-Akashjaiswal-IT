...
export function formatChaiMenu(items) {
  if(!Array.isArray(items) || items === null || items === undefined) return '';
  return items.filter(e => e.price > 0 && typeof e.name === 'string' && e.name !== '').map(e => `${e.name.toUpperCase()} - Rs.${e.price}`).join(' | ')
}
